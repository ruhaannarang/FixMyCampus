...
export const WardenDashboard = ({ complaints, onSelect, onNavigate, onVote }: Props) => {
  ...
  const filtered = complaints.filter((c) => {
    ...
    if (search && !`${c.title} ${c.id} ${c.student}`.toLowerCase().includes(search.toLowerCase()))
      return false;
    return true;
  });
  ...
  const handleApprove = (id: string) => toast.success(`Complaint ${id} approved`);
  const handleReject = (id: string) =>
    toast.error(`Complaint ${id} rejected`, { description: "Student has been notified." });
              ...
              <div className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                >
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    {c.student} · {c.hostel} · Room {c.room} · {c.createdAt}
                  </p>
                </div>
                {c.status === "pending" && (
                ...
                {c.status === "approved" && (
                  <Button
                    size="sm"
                    onClick={() => toast.success(`${c.id} marked resolved`)}
